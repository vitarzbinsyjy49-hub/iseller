# Канал и прайс-посты

> Читать, когда работаешь с каналом `@isellerhub`, прайс-постами или
> инфо-постами. Инвариант в `CLAUDE.md`.

Канал `@isellerhub`, id `-1003998743702`, бот в нём администратор.

**Модель:** пост публикуется ОДИН раз и дальше редактируется на том же
`message_id`. У каждого раздела стабильный `slug` — он связывает раздел каталога
с конкретным сообщением. Потерять slug = потерять сообщение и выложить дубль.

| файл | ответственность |
|---|---|
| `services/price_posts.py` | чистая генерация текста и клавиатур, без сети и БД |
| `services/price_channel.py` | состояние: что опубликовано, что изменилось |
| `services/info_posts.py` | редактируемые посты (гарантия, доставка) + конструктор кнопок |
| `services/telegram_publisher.py` | отправка/правка, retry, 429 |
| `services/telegram_bot.py` | обработчики команд, `build_reply` без сети |

Источник прайса — **каталог в БД**, не XLSX: пост обязан совпадать с тем, что
покупатель видит в Mini App. XLSX — только для импорта.

Каждый пост фиксируется в БД сразу после успеха. Если пачка упрётся в лимит
Telegram на середине, уже полученные `message_id` уцелеют и повторный прогон не
создаст дублей.

Инфо-посты с маркером `[уточнить]` система публиковать отказывается — забытый
черновик в канале хуже отсутствия поста.

## Новый пост — по умолчанию rich

«Красивый пост» у владельца — rich-формат: `<h3>`, картинка `<img>` внутри,
лид, `<blockquote>`, раскрывающиеся `<details>` с `<table>`, `<footer>`.
Образцы: `artifacts/channel-posts/*.md`, пост №44 (`launch_roadmap_2026_09`),
№65 (`info_iphone18_instock`, скрипт `post_iphone18_instock.py`). Фото с
подписью — только по явной просьбе: **фото-пост в rich правкой не
превращается**, 24.09.2026 пришлось публиковать заново и просить владельца
удалить старое сообщение руками (удалять посты сами мы не можем).

## Кнопки в канале

`web_app`-кнопки Telegram разрешает **только в личном чате с ботом**. В канале
сообщение с такой кнопкой отвергается ЦЕЛИКОМ (`BUTTON_TYPE_INVALID`) — то есть
пост просто не публикуется. В канале только `url`-кнопки: ведут на
`t.me/<bot>?start=<payload>`, бот ловит payload и открывает нужный экран Mini App
уже настоящей web_app-кнопкой.

## Медиа в канал — только multipart, никогда URL-ссылкой

**Подтверждено вживую A/B-тестом на `@isellerhub` 19.08.2026.** Telegram
принципиально отказывается сам скачивать медиа с нашего боевого домена
(`*.sslip.io` — бесплатный wildcard-DNS): `send_photo(photo=<URL>)` отвечает
`Bad Request: wrong type of the web page content`, хотя ответ сервера на этот
URL идеальный (200, верный `Content-Type`, без редиректа). Тот же файл,
отправленный multipart-загрузкой (байты в теле запроса) — `200 OK` мгновенно.
Похоже, `*.sslip.io` не в белом списке доменов, с которых Telegram готов сам
фетчить медиа для ботов — дело не в контенте и не в коде отдачи файла.

Значит: **любая своя загрузка (`/api/uploads/...`) уходит в Telegram
multipart-байтами, никогда строкой-URL** — и для обычных постов (`sendPhoto`),
и для rich-контента. Внешние URL (чужой CDN, не наш домен) Telegram по-прежнему
скачивает сам без проблем — тот путь не трогаем.

- `uploads.local_path_for_url()` (`app/core/uploads.py`) резолвит
  относительный или уже абсолютизированный URL нашей загрузки в файл на диске;
  `None` — для внешних URL и для случая, когда файла уже нет (тогда, как и
  раньше, уходит просто URL — единственный доступный fallback).
- `send_photo`/`publish_post` — через общий `_dispatch_photo()`
  (`telegram_publisher.py`): своя загрузка идёт в поле `files={"photo": ...}`
  httpx-запроса, `chat_id`/`caption`/`reply_markup` — обычными полями формы.
- `call()` при `files=` шлёт `data=`+`files=` (multipart) вместо `json=`.
  Вложенные объекты (`reply_markup`, `rich_message`) в multipart сами себя не
  сериализуют — `_multipart_data()` превращает их в JSON-строку, а `bool` — в
  `"true"/"false"` (иначе httpx отдаст питоньи `"True"/"False"`, которые
  Telegram не поймёт).
- Файл читается в байты (`read_bytes()`), не передаётся открытым
  файл-объектом: `call()` при 429/сетевом сбое повторяет `httpx.post` с теми
  же kwargs, а открытый файл на втором проходе был бы уже дочитан до конца.

## Rich-контент (Bot API 10.1 sendRichMessage, media — Bot API 10.2)

Опционально: у `ChannelPost` есть `rich_html`. Когда заполнено — ПОЛНОСТЬЮ
заменяет `body` при публикации, через `send_rich_message`/`edit_rich_message`
(`telegram_publisher.py`) вместо `send_message`/`send_photo`/`edit_message`/
`edit_caption`. `editMessageText` — одна и та же ручка Bot API для обычного и
rich-текста (принимает `rich_message` вместо `text`), поэтому модель «опубликовать
один раз, дальше редактировать на том же message_id» работает без изменений.

- `rich_html` идёт в `rich_message.html` — тот же "Rich HTML style", что и
  `parse_mode=HTML`, плюс `<table>`, `<h1>`–`<h6>`, `<hr/>`,
  `<details><summary>…</summary>…</details>`, `<footer>`, `<blockquote>`,
  `<aside><cite>`, `<img>`/`<video>`/`<audio>`.
- **Своя загрузка в `<img>`/`<video>`/`<audio> src`** (см. раздел выше про
  multipart) — `_prepare_rich_media()` вырезает её из `src` и переносит в
  `InputRichMessage.media` (Bot API 10.2, добавлено 14.07.2026): `src`
  заменяется на `tg://photo?id=…`/`tg://video?id=…`/`tg://audio?id=…`, а сам
  файл летит multipart-вложением под именем `attach://<id>_<имя-файла>` —
  единственный задокументированный способ вставить в rich-контент медиа не
  по HTTP(S)-URL. Раньше (до фикса) относительный `src` просто абсолютизировался
  через `public_image_url()`, и Telegram пытался скачать его сам — с боевого
  домена это больше не работает никогда, см. раздел выше.
- **Внешний URL** (чужой CDN, не наш `/api/uploads/...`) в `src` не трогаем —
  Telegram скачивает его сам, как и раньше; `Media blocks support only HTTP
  and HTTPS URLs` (документация) — это ограничение относится именно к прямому
  URL в `src`, а не к пути через `tg://…?id=` + `media`.
- Если своей загрузки на диске уже нет (файл удалён, но `src` остался), а
  `MINI_APP_URL` не настроен — публикация падает с понятной ошибкой вместо
  того, чтобы уйти в канал без картинки (тот же контракт, что был у
  `_absolutize_rich_media` раньше).
- У rich-сообщений нет режима "фото с подписью" (`sendPhoto`) — картинка идёт
  тегом `<img>` прямо внутри `rich_html`; `image_url` для rich-поста не
  используется совсем.
- Лимит — `MAX_RICH_MESSAGE_LENGTH = 32768` символов (документированный лимит
  Telegram), отдельно от `MAX_MESSAGE_LENGTH`/`MAX_CAPTION_LENGTH` обычных
  постов.
- Кнопки — те же `build_keyboard()`/`resolve_button()`, ничего не меняется:
  `reply_markup` у `sendRichMessage`/`editMessageText` принимает тот же
  `{"inline_keyboard": [...]}`.
- Автоматических тестов, бьющих в настоящий Bot API, в проекте нет (все тесты
  мокают `httpx.post`) — перед публикацией непроверенной rich-разметки в
  `@isellerhub` стоит один раз вручную отправить её в тестовый чат.

## Страница прайса может ПЕРЕСТАТЬ генерироваться — она не исчезнет сама

Секция режется на посты по длине текста, поэтому число страниц плавает от
чего угодно: цены, длины названий, вёрстки строки. Рост страниц система
обрабатывает (`action == 'create'`), а вот **сокращение — нет**.

Поймано 08.09.2026: подзаголовки моделей укоротили строки, iPhone снова
уместился в две страницы вместо трёх, и `price_iphone_p3` просто выпал из
`build_plan`. Сообщение при этом осталось в канале и держало устаревшие цены
на девять айфонов, которые уже показаны выше. Молча и без единой ошибки в
логах — план показывал 13 позиций вместо 14, и всё.

**С 24.09.2026 это делает `price_channel.retire_orphan_pages()`** — звать после
`apply_plan` (сама находит выпавшие страницы и переписывает их на отсылку с
кнопкой на первую часть). Ручная проверка ниже — на случай, если забыли.

**Проверять после КАЖДОЙ правки вёрстки прайса:** сравнить slug'и из
`build_plan` со slug'ами опубликованных постов (`select slug from
channel_posts where slug like 'price_%'`). Разница — осиротевшие страницы.

**Удалять их нельзя.** Строка в БД связывает slug с `message_id`, и если
раздел снова разрастётся, система переиспользует ЭТО сообщение вместо
публикации дубля. Правильное действие — переписать текст на короткую отсылку
(`edit_message`), связь оставить.

Мелочь на будущее: `ChannelPost.reply_markup` лежит то списком рядов, то
объектом Bot API с ключом `inline_keyboard`. Читать надо оба вида, иначе при
правке поста с него слетают кнопки.
