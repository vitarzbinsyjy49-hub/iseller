...
AI Gateway (Mac mini M4 Pro, приватная сеть: Tailscale)      [ai-gateway/main.py]
   │  X-API-Key · payload ≤64KB · rate limit · semaphore(1)+очередь · timeout 45s (inference)
   ▼
Ollama (127.0.0.1:11434, наружу не торчит) · qwen3:14b · format=JSON Schema, think:false
   ▼
VPS · backend
...
- **System prompt версионирован**: `backend/app/prompts/ai_seller_system_v2.md`,
  переключение через `AI_SYSTEM_PROMPT_VERSION` без правки кода.
...
| backend | `app/prompts/ai_seller_system_v2.md` | системный промпт (versioned, default v2) |
| backend | `app/api/ai.py` | маршрутизация AI_PROVIDER (без слома legacy) |
| gateway | `ai-gateway/main.py` | FastAPI: auth, лимиты, Ollama |
| frontend | `src/pages/AiSearch.tsx` | история (≤10) + timeout 75с |
...
AI_GATEWAY_TIMEOUT_SECONDS=65   # очередь(10с)+inference(45с)+сеть
AI_MAX_HISTORY_MESSAGES=10
AI_MAX_PRODUCT_CANDIDATES=12
AI_FALLBACK_ENABLED=true
AI_SYSTEM_PROMPT_VERSION=v2
```
...
## Ограничения v1
- Streaming не включён (короткие ответы + format=JSON Schema, think:false; можно добавить позже).
