...
Пилюля отрывается от нижней кромки, и на величину зазора должно вырасти `--bottom-nav-height`. От неё уже зависят `.cta-dock`, `.cart-dock`, `.toast-dock`, `.pb-nav` и `.pb-cta` — все едут следом сами. Одновременно правится `bottomNavStack()` в `viewport.ts`: эта функция — зеркало той же геометрии, живущее только ради тестов, и рассинхрон с CSS сделает её ложью.
