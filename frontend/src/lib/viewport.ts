...
export type OrientationLockSources = {
  ...
  supported: boolean;
  /** Telegram.WebApp.isOrientationLocked — уже закреплено. */
  alreadyLocked: boolean;
  /** Сейчас портретная ориентация. */
  portrait: boolean;
};

/** Пора ли закреплять ориентацию.
 *
 *  Зачем вообще. Альбомная раскладка приложения не проектировалась: карточки,
 *  шторки и нижний ряд рассчитаны на узкий экран, и поворот телефона даёт
 *  сломанный вид вместо другого вида. Закрепить ориентацию честнее, чем
 *  чинить раскладку, которой никто не пользуется.
 *
 *  Почему условие, а не просто вызов при старте. `lockOrientation()` закрепляет
 *  ТЕКУЩУЮ ориентацию — задать желаемую API не позволяет. Значит вызов вслепую
 *  на старте закрепил бы альбомную у того, кто открыл приложение с уже
 *  повёрнутым телефоном, то есть заморозил бы ровно ту раскладку, от которой мы
 *  уходим, и выйти из неё было бы уже нельзя. Поэтому закрепляем только когда
 *  портрет наблюдается по факту; если человек начал в альбомной, ждём первого
 *  поворота в портрет и закрепляем тогда.
 *
 *  `alreadyLocked` — не оптимизация: повторные вызовы бессмысленны, а признак
 *  ещё и служит условием остановки для подписки на поворот.
 */
export function shouldLockOrientation(s: OrientationLockSources): boolean {
  return s.supported && !s.alreadyLocked && s.portrait;
}
