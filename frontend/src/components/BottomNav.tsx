  ...
  return (
    ...
    <nav
      ...
      {/* Выдача — растёт ВВЕРХ от ряда, а не отдельным слоем поверх экрана.
          ...
          предел, что был у прежнего SearchOverlay. Экран под ней виден и
          доступен: в отличие от прежней панели, здесь нет затемняющей
          подложки на весь экран — закрывают явные «Отмена» и протяжка вниз,
          Escape и кнопка «назад» Telegram (см. эффекты выше). */}
      {searchOpen && (
        <div
          ref={panelRef}
          role="dialog"
