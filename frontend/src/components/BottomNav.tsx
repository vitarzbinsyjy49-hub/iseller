...
/** Подпись строки поиска в покое и при выключённом движении. Именно она
 *  объясняет, что строка ищет НЕ только товары, — подсказки её дополняют, а не
 *  заменяют. */
const SEARCH_PLACEHOLDER = "Товары и разделы";

/** Что строка печатает сама, пока в неё не начали писать.
 *
 *  Подобраны так, чтобы каждая фраза показывала РАЗНОЕ умение строки: название
 *  модели, житейскую формулировку без модели, раздел приложения. Список из
 *  одних «iPhone / MacBook / AirPods» научил бы человека только тому, что уже
 *  написано в чипах «Часто ищут» ниже.
 *
 *  Ничего, чего строка не умеет: каждая фраза обязана давать непустой результат
 *  либо в каталоге, либо в разделах (lib/appSections.ts). Подсказка, ведущая в
 *  пустой экран, — обещание, которого приложение не выполняет. */
const SEARCH_HINTS = [
  "смартфоны",
  "наушники",
  "мои заявки",
...
export default function BottomNav() {
  ...
  // Бегущая подсказка: поле само показывает, что в него можно написать.
  // Фразы разного РОДА намеренно — товар, свойство, раздел приложения: строка
  // ищет и по каталогу, и по разделам (searchAppSections), и подсказка из
  // одних названий моделей скрывала бы половину её умения.
  useTypewriterPlaceholder(
    inputRef,
    SEARCH_HINTS,
    // Только пока поле пустое: поверх набранного текста подсказки не видно, а
    // кадры идут — это трата ровно в тот момент, когда человек печатает и
    // подтормаживание заметнее всего.
    searchOpen && query === "",
    SEARCH_PLACEHOLDER,
  );
  ...
  return (
    ...
    <nav
      ...
      {/* Ловец тапа мимо панели. Панель занимает часть экрана, остальное
          ...
          Рендерится ДО панели и ряда в DOM, но одного этого НЕДОСТАТОЧНО, и
          здесь ровно на этом обожглись: порядок в DOM решает только среди
          соседей одного слоя отрисовки. Подложка позиционирована (fixed), а
          панель была статичной — и по правилам отрисовки (CSS 2.1, Appendix E:
          непозиционированные блоки на шаге 4, позиционированные на шаге 8)
          подложка накрывала панель целиком, собирая все её нажатия. Поэтому
          .nav-search-panel обязана иметь position (index.css), а .nav-pill/
