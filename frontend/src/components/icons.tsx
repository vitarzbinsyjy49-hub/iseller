...
/** Толщина обводки по умолчанию — одна на всё приложение.
 *
 *  Замер верха главной до этой ревизии дал 1,7 / 1,8 / 2 / 2,2 в одном ряду.
 *  Разницу между 1,7 и 1,8 не назовёт никто, но глаз видит, что элементы «не
 *  из одного набора» — из таких мелочей и складывается ощущение самоделки.
 *  Значение задаётся здесь, а не приходит с каждого места вызова: пока его
 *  можно передать снаружи «на глаз», разнобой возвращается сам.
 *
 *  Проп strokeWidth оставлен для редких осознанных исключений (крупная иконка
 *  в пустом состоянии, где 1,8 выглядит ниткой), но в рядовой разметке его
 *  указывать не нужно. */
const STROKE = 1.8;

export function Icon({
  name, className = "h-5 w-5", strokeWidth = STROKE, filled = false,
