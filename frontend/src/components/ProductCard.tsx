...
function CardCartControl({ card, onOpen }: { card: TCard; onOpen: () => void }) {
  ...
  if (!orderable) {
    return (
      ...
      >
        {/* «Предзаказ», а не «Оформить предзаказ»: в плитке под кнопку те же
            112px, и глагол съедал их, ничего не сообщая. На карточке товара
            кнопка остаётся полной («Оформить предзаказ») — там ширина целого
            экрана и кнопка единственная, ей положено называть действие.
            Разница между плиткой и деталкой здесь осознанная, а не разъезд. */}
        {card.price_note ? "Предзаказ" : "Узнать о поступлении"}
      </button>
    );
  }
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-3">
        ...
        {/* Цена или её отсутствие. price_note приходит с backend непустым ровно
            ...
            ОДНА строка, высотой ровно как у цены (22px = leading у text-title),
            и truncate, который это гарантирует. Раньше подпись шла с leading-5
            и без ограничения: в мобильной плитке под текст остаётся ~144px,
            текст переносился на две строки и абзац занимал 40px вместо 22px.
            Плитки в сетке растянуты по самой высокой в ряду (h-full), поэтому
            каждый ряд с предзаказом становился на 18px выше соседних — ряды
            «ехали» без видимой причины. Замерено: 348px против 330px.

            Ограничение держится в ДВУХ местах и оба нужны. Здесь — чтобы
            сетку нельзя было сломать в принципе; в backend (availability.
            PRICE_NOTE) — чтобы подпись влезала и не превращалась в многоточие.
            Одного truncate мало: он чинит раскладку ценой смысла. */}
        {card.price_note ? (
          <p className="mt-1.5 truncate text-[14px] font-semibold leading-[1.375rem]">
            {card.price_note}
          </p>
        ) : (
        ...
        {card.preorder_eta ? (
          // «с 23 октября», а не «Ожидается 23 октября». Замерено на узком
          // телефоне (320px): под текст в плитке остаётся 112px, и длинная
          // форма занимала ДВЕ строки — 36px вместо 18. Глагол здесь ничего
          // не добавляет: рядом стоит «Цена по запросу» и кнопка «Предзаказ»,
          // из них уже понятно, что товар ещё не приехал, а предлог «с»
          // отвечает ровно на оставшийся вопрос — когда.
          // truncate — та же страховка, что у цены: дата приходит из админки
          // строкой, и длинную формулировку туда впишут рано или поздно.
