...
#:
#: Подпись обязана влезать в ОДНУ строку на месте цены в плитке каталога.
#: Прежняя («Цену уточнит менеджер», 21 символ) в мобильной плитке переносилась
#: на две строки: под текст там остаётся ~144px, и абзац занимал 40px вместо
#: 22px. Плитки в сетке растянуты по самой высокой, поэтому КАЖДЫЙ ряд с
#: предзаказом становился на 18px выше соседних — ряды визуально «ехали».
#: Вёрстка теперь режет строку принудительно (truncate), так что длинный текст
#: сетку больше не сломает, но превратится в многоточие. Держать в пределах
#: ~16 символов.
PRICE_NOTE = {
    "preorder": "Цена по запросу",
}
