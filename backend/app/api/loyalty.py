...
def my_loyalty(
    ...
    # Условия следующей покупки считает сервер: акция зависит от журнала
    # покупок и от даты, и вычислять её во фронте значило бы завести вторую
    # копию правила, которая разъедется с первой.
    data["next_purchase"] = loyalty.next_purchase_terms(db, user.id)
    return data
