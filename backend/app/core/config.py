...
class Settings(BaseSettings):
    ...
    # Дневной потолок платных вызовов модели на пользователя — защита от того,
    # что один бесплатный Telegram-аккаунт выжигает бюджет на API непрерывной
    # долбёжкой в рамках минутного лимита (10/мин без дневного потолка — это
    # ~$57/день с одного аккаунта на Haiku). При исчерпании запрос НЕ падает
    # 429м — молча уходит в бесплатный ответ по каталогу (см. api/ai.py).
    AI_CHAT_DAILY_LIMIT_PER_USER: int = 50
    ...
    # Фото к отзыву — та же угроза, что у заявки выше, и по той же причине:
    # ручка открыта любому авторизованному, а Telegram-аккаунт создаётся за
    # минуту. Без потолка один аккаунт льёт по 8 МБ, пока на VPS не кончится
    # диск — вместе с диском ложатся и витрина, и бот. Значение с запасом на
    # честное использование: в отзыве до 10 фото (MAX_REVIEW_PHOTOS), то есть
    # это три полных отзыва в сутки с правками.
    REVIEW_PHOTO_DAILY_LIMIT_PER_USER: int = 30
